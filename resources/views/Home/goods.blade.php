	...
	<head>
		<meta charset="UTF-8">
		<title>{{$goods->title}}_{{config("web.title")}}</title>
		<meta name="keywords" content="{{$goods->title}}" />
		<meta name="description" content="{{$goods->info}}" />
	...
	<body>
		...
		<div class="container centers">
			<div class="mianbao">
				<a href="/">首页</a> &gt; <a href="#">商品详情</a>&gt; {{$goods->title}}
			</div>
			<div class="action">
				<div class="ac-left">  
					<!-- 橱窗图上部  空白位-->
					...
					<div class="imgs" id="imgSlider">
						...
						<ul class="img-d" id="imgBig">
							...
							<li id="bigim{{$key}}" style="z-index:1;filter: alpha(opacity=100); opacity: 1;"><a href="javascript:;">
...
<div class="mainsss">
	...
	<div class="box-rt">
		...
		<div id="content">
			...
			<ul>{!!$goods->text!!}</ul>
			<!-- 配置信息 -->
			<ul style="display: none;">{!!$goods->config!!}</ul>
			@if ($arr['commentTot'])	                                                          
					...
					<div id="pager_divdiv3" class="ns_discusspager">
						<div class="ns_comment-inner" id="sppl_box">
							<dl id="sppl_title"> 
								...
								<dd class="sppl_total active" data-level="0">全部评价(<span>{{$arr['commentTot']}}</span>)
								</dd>  
								<dd class="sppl_good" data-level="3">好评(<span>{{$arr['goodTot']}}</span>)</dd> 
								<dd class="sppl_medium" data-level="2">中评(<span>{{$arr['midTot']}}</span>)</dd>  
								<dd class="sppl_total" data-level="1">差评(<span>{{$arr['badTot']}}</span>)
								</dd> 
							</dl>
							<dl id="sppl_list" class="clearfix">
								@foreach ($comment as $discuss)
								<dd>
									<div class="sppl_top">  
										<span style="color: red;">{{str_repeat("★", $discuss->start)}}{{str_repeat("☆", 5 - $discuss->start)}}</span>   
										<span class="sppl_username">{{$discuss->name}}</span>   
										<span class="sppl_time">{{date("Y-m-d H:i:s",$discuss->time)}}</span>  
									</div>  
									<div class="sppl_user_box">
										<p><a class="clickpl" uuid="9ffce34a-deda-483c-8754-833b29421f76" href="javascript:void(0);">{{$discuss->text}}  </a>
										</p> 
									</div>      
								</dd>
								@endforeach
