...
<head>
	<meta charset="UTF-8">
    <title>{{config('web.title')}}_结算中心</title>
...
<div class="box-01">
    ...
    <div id="addressList-SH">
        ...
        @foreach ($addr as $site)
            <p>
                <input type="radio" checked name="aid" value="{{$site->id}}">
                <span>{{$site->sname}}</span>
                <span>{{$site->stel}}</span>
                <span>{{$site->addr}}</span>
                <span>{{$site->addrInfo}}</span>
            </p>
...
<div class="list" id="52771" style="border-bottom: none;">
    ...
    <input type="hidden" name="nums[]" value="{{$shop['num']}}">
    <input type="hidden" name="prices[]" value="{{$shop['goodsInfo']->price}}">
    <p class="list-a">
        <a href="/goods/{{$shop['id']}}"><img title="{{$shop['goodsInfo']->title}}" src="/Uploads/goods/{{$shop['goodsInfo']->img}}" width="110px" height="110px"></a>
    </p>
    <p class="list-b">
