...
@if ($shop)
             ...
             <tbody>
                ...
                <tr class=" active" gcode="52770" stock="18">
                    <td width="57" class="bc_table_sel">
                        <input type="checkbox" name="goods[]" value="{{$car['id']}}" />
                    </td>
                    <td width="148" class="bc_table_img">
                        <a href="/goods/{{$car['id']}}" target="_blank">
                            <img src="/Uploads/goods/{{$car['goodsInfo']->img}}" alt="">
                        </a>
                        <div class="bc_zhezhao"></div>
                    </td>
                    <td width="257" class="bc_proname bc_name_gift">
                        <a style="color: red;" href="/goods/{{$car['id']}}" target="_blank">{{$car['goodsInfo']->title}}</a>
                    </td>
                    <td width="163"><a href="/goods/{{$car['id']}}" target="_blank">{{$car['goodsInfo']->info}}</a> </td>
                    <td width="152" style="position:relative;">
                        <span class="current_price">{{$car['goodsInfo']->price}}</span>
                        <!--满减-->
                     </td>
                    <td width="152" text-align:center>
                        <label class="i_box clearfix">
                            ...
                            <input class="pro_num J_input" type="text" value="{{$car['num']}}" style="ime-mode: disabled;" val="{{$car['num']}}">
                            <input class="pro_add J_add" money="{{$car['goodsInfo']->price}}" ids="{{$car['goodsInfo']->id}}" type="button" value="+">
                        </label>
                    </td>
                    @php
                        ...
                        $tot += $money;
                    @endphp
                    <td width="152" class="bc_red">{{$money}}</td>
                    <td width="117" style="text-align:left">
                        <a href="javascript:;" style="color: #000" title="删除" onclick="deleline(this,{{$car['goodsInfo']->id}})">删除</a>
                                                <br>
    ...
    <div class="bc_num clearfix">
        ...
        <ul class="bc_num_fr">
            <li>合计：<span class="bc_red" id="totalmoneyf"><span id="heji">{{$tot}}</span> 元</span></li>
...
<script>
    ...
    $(".J_minus").click(function(){
        ...
        obj = $(this);
        // 发送ajax请求
        $.post("/CarMinus",{"id":id,"_token":"{{csrf_token()}}"},function(data){
            if (data) {
                // js修改输入框的数量
                num = obj.next().val();
                num = Number(num);
