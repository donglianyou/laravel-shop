            ...
            <div class="left-menu">
              ...
              <div class="left-xiang">
                ...
                <div class="left-tu">
                  ...
                  <a href=""><img class="classification_img" title="{{$rightAds->title}}" src="/Uploads/ads/{{$rightAds->img}}"></a>
                  @endforeach
                </div>
              </div>
              </div>
    ...
    <div class="center">
      <ul class='imgs'>
        @foreach ($slider as $key => $value)
          @if ($key==0)
            <li class='active'><a href="{{$value->href}}" target="_blank"><img src="/Uploads/lun/{{$value->img}}" alt="{{$value->title}}"></a></li>
          @else
            <li><a href="{{$value->href}}" target="_blank"><img src="/Uploads/lun/{{$value->img}}" alt="{{$value->title}}"></a></li>
          @endif
        @endforeach
      </ul>
      <ul class='nums'>
     ...
     <div class="bao"> 
      ...
      <div class="boo">
        <ul class="bo-1"> 
          ...
          @if ($key>1)
          <li style="width: 252px; height: 159px;"><a target="_blank" href="{{$value->href}}"><img src="/Uploads/ads/{{$value->img}}" alt="{{$value->title}}"></a>
            @else
            <li><a target="_blank" href="{{$value->href}}"><img src="/Uploads/ads/{{$value->img}}" alt="{{$value->title}}"></a>
              @endif
            </li>
            @endforeach
          </ul>
          ...
          <ul>
            ...
            <li style="width: 198px; height: 297px;">
              <a target="_blank" href="/goods/{{$good->id}}"> 
                <img width="198px" height="230px" title="{{$good->title}}" src="/Uploads/goods/{{$good->img}}">
              </a>
              <p class="star_name">
                <a target="_blank" title="{{$good->title}}" href="/goods/{{$good->id}}">{{$good->title}}</a>
              </p>
              <p class="star_ad">
                <a target="_blank" href="/goods/{{$good->id}}">{{$good->info}}</a>
              </p>
              <p class="star_price"><a target="_blank" href="/goods/{{$good->id}}">{{number_format($good->price)}}元</a></p>
            </li>
            @endforeach
          <div class='clear'></div>
        </ul>
      ...
      <div class='container lou'>
        ...
        @foreach ($type as $key => $floor)
          @if ($floor->is_lou)
            ...
            <div class="lenovo">
             <div class="title"> 
               <h3><span>{{$louNum}}F</span><em>{{$floor->name}}</em></h3>
               <div class="jieshao">
                @foreach ($floor->son as $son)
                  <a target="_blank" href="" title="{{$son->name}}">{{$son->name}}</a>
                @endforeach
            ...
            <div class="rong">
             <div class="zuo">
                <a target="_blank" href="" title="{{$floor->leftAds->title}}"> <img width="240" height="535" src="/Uploads/ads/{{$floor->leftAds->img}}"></a>
             </div>
             <div class="you">
              @foreach ($floor->goods as $key => $fgoods)
               <div class="you{{++$key}} good">
                  <a target="_blank" href="/goods/{{$fgoods->id}}" title="{{$fgoods->title}}"> <img width="164" height="164" src="/Uploads/goods/{{$fgoods->img}}" ></a>
                 <a class="good-jie"target="_blank" href="/goods/{{$fgoods->id}}">{{$fgoods->title}}</a>
                 <a class="good-jie1"target="_blank" href="/goods/{{$fgoods->id}}" title="{{$fgoods->title}}">{{$fgoods->info}}
                 </a>
                 <a class="money"  target="_blank" href="/goods/{{$fgoods->id}}"  title="{{$fgoods->title}}">{{number_format($fgoods->price)}}元
                 </a>
